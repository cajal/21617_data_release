"""
21617 release schema classes and methods
"""
...
#                      {'animal_id': 21617, 'session': 9, 'scan_idx': 8}]  # last scan is static scan
...
class UnitHash(dj.Manual):
    ...
    def fill(self):
        ...
        for key in self.key_source:
            ...
            str_key = '_'.join([str(s).zfill(z) for s,z in zip(key.values(),(2,2,5))])
...
class Activity(dj.Manual):
    ...
    @property
    def key_source(self):
        return meso.Activity.Trace & Segmentation.segmentation_key & self.activity_key & Field
...
class Stimulus(dj.Manual):
    ...
    @property 
    def key_source(cls):
        return (experiment.Scan.proj() & Scan().inhib_scans) - Stimulus
...
class RawDLCPupil(dj.Manual):
    ...
    def fill(cls):
        for key in cls.key_source:
            ...
            # pull fitted pupil information from database
            ...
            cls.insert1({'session':key['session'],
                         ...
                         'pupil_x':raw_pupil_x,
                         'pupil_y':raw_pupil_y,
                         'pupil_times':adjusted_pupil_times},**params)
...
class DLCPupil(dj.Manual):
    ...
    def fill(cls):
        for key in cls.key_source:
            ...
            # fetch raw pupil info from RawDLCPupil
