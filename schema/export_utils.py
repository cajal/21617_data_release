...
def reconstruct_refresh_rate_trial(trial_key,stim_type,intertrial_time,
    ...
    assert np.all(np.abs(np.diff(flip_times)-(1/frame_rate)) < tol), 'frame rate deviation > 1 ms detected'
    ...
    if upsample_ratio > 1:
        ...
        idx2ft = interp1d(np.arange(0,len(flip_times[0])*upsample_ratio,upsample_ratio),
                          flip_times[0],kind='linear') 

        # linearly interpolated intratrial flip times at refresh rate
        intratrial_flip_times = idx2ft(np.arange(0,(len(flip_times[0])-1)*upsample_ratio))

        # last flip and intertrial period at refresh rate
        intertrial_frames = (np.round(intertrial_time * est_refresh_rate)-1).astype(int)
        intertrial_flip_times = np.linspace(flip_times[0][-1],
                                            flip_times[0][-1]+intertrial_time,
                                            intertrial_frames+2)[0:-1]

        # concatenate
        _flip_times = np.concatenate((intratrial_flip_times, intertrial_flip_times))

        # repeat intratrial stim frames to fill refresh rate
        intratrial_movie = np.repeat(movie[:,:,:-1],upsample_ratio,axis=time_axis)

        # repeat last stim frame for intertrial period at refresh rate
        intertrial_movie = np.repeat(movie[:,:,-1:],intertrial_frames+1,axis=time_axis)

        # concatenate
        _movie = np.concatenate((intratrial_movie,intertrial_movie),axis=time_axis)

    if upsample_ratio == 1:
        # intertrial period at refresh rate
        intertrial_frames = (np.round(intertrial_time * est_refresh_rate)-1).astype(int)
        intertrial_flip_times = np.linspace(flip_times[0][-1],
                                            flip_times[0][-1]+intertrial_time,
                                            intertrial_frames+2)[1:-1]

        # concatenate
        _flip_times = np.concatenate((flip_times[0],intertrial_flip_times))

        # repeat last stim frame for intertrial period at refresh rate
        intertrial_movie = np.repeat(movie[:,:,-1:],intertrial_frames,axis=time_axis)

        # concatenate
        _movie = np.concatenate((movie,intertrial_movie),axis=time_axis)

    return _flip_times,_movie
...
def resample_stim(scan_key, target_times=None, target_size = (90,160), time_axis=2, 
    ...
    assert np.all(np.abs(np.round(intertrial_times * est_refresh_rate) - (intertrial_times*est_refresh_rate)) < tol), \
                'intertrial frame rate deviation > 1 ms detected'
